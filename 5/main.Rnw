...
Реализовать алгоритм вычисления производной информационной матрицы Фишера по
компонентам вектора входного сигнала.
...
dM <- function(Phi, Psi, Gam, H, Q, R, X_0, P0, th, u)
{
	# 1
	dPhi <- matderiv(Phi, th)
	dPsi <- matderiv(Psi, th)
	...
	for (t in 0:(N-1)) {
		# 3
		# k, X_Ap, PhiA, PsiA, u, dX_0, Phi, dPhi, X_0, Phi_dX_0
		if (t == 0)
		{
			# dX_Ap
			for (j in 1:r) {
		...
		else
		{
			for (j in 1:r) {
				for (tau in 0:(N-1))
				{
					...
					dX_Ap[j,tau+1,] <- PhiA %*% dX_Ap[j,tau+1,] + PsiA %*% du
				}
			}
			...
			for (tau in 0:(N-1)) {
				...
				for (a in 1:s) {
					...
					for (b in 1:s) {
						...
						# TODO: X_Ap %*% t(dX_Ap_) + dX_Ap_ %*% t(X_Ap)
						#X_Ap__dX_Ap_t__dX_Ap__X_Ap_t

						I[a,b] <- Sp(C0 %*% (X_Ap %*% t(dX_Ap_) + dX_Ap_ %*% t(X_Ap)) %*%
												 t(C0) %*% t(dH[[b]]) %*% invB %*% dH[[a]])

						II[a,b] <- Sp(C0 %*% (X_Ap %*% t(dX_Ap_) + dX_Ap_ %*% t(X_Ap)) %*%
													t(Cb) %*% t(H) %*% invB %*% dH[[a]])

						III[a,b] <- Sp(Ca %*% (X_Ap %*% t(dX_Ap_) + dX_Ap_ %*% t(X_Ap))
													%*% t(C0) %*% dH[[b]] %*% invB %*% H)

						IV[a,b] <- Sp(Ca %*% (X_Ap %*% t(dX_Ap_) + dX_Ap_ %*% t(X_Ap)) %*%
													t(Cb) %*% t(H) %*% invB %*% H)
					}
				}
				# put in dM
				dMth[j,tau+1,,] <- I + II + III + IV
			}
		}
		Pe <- Pu
	}
