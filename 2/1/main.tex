...
\begin{equation*}
\begin{split}
	\frac{\partial \chi(\theta)}{\partial \theta_k} = \sum\limits_{k=0}^{N-1}
	\left[ \frac{\partial \eps(t_{k+1})}{\partial \theta_k} \right]^T
	B^{-1}(t_{k+1}) \left[ \eps(t_{k+1}) \right] - \\
	- \frac{1}{2}
	\sum\limits_{k=0}^{N-1} \left[ \eps(t_{k+1}) \right]^T B^{-1}(t_{k+1})
	\frac{\partial B(t_{k+1})}{\partial \theta_k} B^{-1}(t_{k+1}) \eps(t_{k+1}) +
	\\ + 
	\frac{1}{2} \sum\limits_{k=0}^{N-1} \Sp \left[ B^{-1}(t_{k+1})
	\frac{\partial B(t_{k+1})}{\partial \theta_k} \right]. 
\end{split}
\end{equation*}
...
\begin{enumerate}
\item Для заданного $\theta$ найдем: $F, \pd{F}{\theta_k}, Q, \pd{Q}{\theta_k},
C, \pd{C}{\theta_k}, R, \pd{R}{\theta_k}, G, \pd{G}{\theta_k}, \overline{x}(0),
\pd{\overline{x}(0)}{\theta_k}, H, \pd{H}{\theta_k}$, $P(0),
\pd{P(0)}{\theta_k}$, $k = 1, 2, \ldots, s$. 

\item Положим $k = 0$; $\pd{\chi(\theta)}{\theta_k} = 0$; $P(0|0) = P(0)$;
$\pd{P(0|0)}{\theta_k} = \pd{P(0)}{\theta_k}$, $k = 1, 2, \ldots, s$;
$\Delta_2'$ = 0.
...
\[
	P(t_{k+1}|t_{k+1}) = \left[ I - K(t_{k+1}) H \right] P(t_{k+1}|t_k)
\]
\item Найдем $\pd{P(t_{k+1}|t_k)}{\theta_k}$, $\pd{B(t_{k+1})}{\theta_k}$,
$\pd{K(t_{k+1})}{\theta_k}$, $\pd{P(t_{k+1}|t_{k+1})}{\theta_k}$ для всех
$k = 1, 2, \ldots, s$ по формулам, вытекающим из уравнений фильтра Калмана:
\begin{equation*}
\begin{split}
	\pd{P(t_{k+1}|t_k)}{\theta_k} = \pd{F}{\theta_k} P(t_k|t_k) F^T + F
	\pd{P(t_k|t_k)}{\theta_k} F^T + F P(t_k|t_k) \pd{F^T}{\theta_k} + \\ +
	\pd{G}{\theta_k} Q G^T + G \pd{Q}{\theta_k} G^T + G Q \pd{G^T}{\theta_k};
\end{split}
\end{equation*}
...
\pdpk{\hat{x}(\estfut)}$ для всех $k = 1, 2, \ldots, s$ по формулам, вытакающим
...
$k = 1, 2, \ldots, s$ и закончим процесс.
