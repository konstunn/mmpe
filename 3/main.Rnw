...
M <- function(Phi, Psi, Gam, H, Q, R, X_0, P0, th, u)
{
	...
	PsiA <- rbind(Reduce(rbind, Psi), dPsi)
	...
	for (k in 0:N-1)
	{
		...
		PhiA <- PhiA(Phi, K_e, Reduce(rbind, dPhi), H, dH)
		...
		# 6
		PhiA <- PhiA(K_p, Phi, Reduce(rbind, dPhi), H, Reduce(rbind, dH))
		...
		Pp_tdH <- lapply(dH, function(dH_i) Pp %*% t(dH))
		...
		for (i in 1:s) {
			for (j in 1:s) {
				...
				IV <- Sp(C(i,n,s) %*% EAp__X_Ap__tX_Ap %*% C(j,n,s) %*% t(H) %*%
								 invBp %*% H)
