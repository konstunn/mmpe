...
\subsubsection{Вспомогательные процедуры}
...
hin <- function(u) {
	...
	a <- head(u, n=-q)
	alen <- length(a)
	h <- p - rep(10 * .Machine$double.eps, length(p))
	h <- c(h, 1 - a)
	h <- c(h, a + 1)
	return(h)
}
...
\subsubsection{Очистка оптимального плана}
...
\subsubsection{Теоретический оптимальный план}
...
\newpage
\subsubsection{Начальный план - субоптимальный план}
