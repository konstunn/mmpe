...
Ознакомиться со структурой стохастической дискретной модели в пространстве
состояний. Реализовать алгоритм вычисления информационной матрицы плана для
...
\begin{enumerate}
	...
	\item Написать программу, реализующую алгоритм вычисления информационной
		матрицы Фишера для стохастических линейных дискретных систем.

	\item Вычислить информационную матрицу многоточечного плана.

	\item Проверить правильность работы программ на стохастических дискретных
		моделях с различными вариантами вхождения неизвестных параметров в уравнения
		состояния, уравнения измерения и начальные условия.
...
M <- function(Phi, Psi, Gam, H, Q, R, X_0, P0, th, u)
{
	...
	for (k in 0:(N-1))
	{
		...
		if (k > 0) {
			...
			EAp <- matrix(0, nrow=n*(s+1), ncol=n*(s+1))
		}
...
<<test>>=
M(Phi, Psi, Gam, H, Q, R, X_0, P0, th, u)
@

Тест 2:

<<mod2, purl=T>>=
Phi <- function(th) matrix(0.5)

Psi <- function(th) matrix(0.9)

Gam <- function(th) matrix(1)

Q <- function(th) matrix(th[1])
